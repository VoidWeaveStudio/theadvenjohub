...
import {
  ...
  getSafeBrowseTarget,
  getStoreUrl,
} from "@/core/wallets/mobileDeeplinks";
...
export function WalletSelectorModal({ isOpen, onClose, onSelect }: WalletSelectorModalProps) {
  ...
  const availableWallets = useMemo(() => {
    ...
    (wallets ?? []).forEach((wallet) => {
      if (wallet.adapter.name === MOBILE_WALLET_ADAPTER_NAME) return;
      ...
      {availableWallets.map((wallet) => {
        ...
        return (
          <button
            ...
            className="w-full flex items-center gap-4 p-3 min-h-[56px] rounded-lg border border-border hover:border-primary/50 hover:bg-surface/50 transition-all disabled:opacity-40 disabled:cursor-not-allowed group"
          >
  ...
  const renderBody = () => {
    if (showMobileHandoff) {
      if (availableWallets.length === 0) return renderMobileHandoff();
    ...
    if (availableWallets.length === 0 && isProbing) return renderProbing();
    if (availableWallets.length === 0) return renderInstallList();
