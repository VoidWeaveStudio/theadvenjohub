...
export function SignEditorModal({ isOpen, onClose, signId, onSubmitText, onSubmitDraw, onNotification }: SignEditorModalProps) {
    ...
    const handleSaveText = async () => {
        ...
        try {
            ...
            onNotification?.(`⚠️ ${err instanceof Error ? t(err.message) : t("g.sign.saveFailed")}`, 3000);
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleSaveDrawing = async () => {
        ...
        try {
            ...
            if (!blob) {
                onNotification?.(t("g.sign.exportFailed"), 3000);
                return;
            }
        ...
        <div className="absolute inset-0 bg-[rgba(6,6,8,0.85)] backdrop-blur-sm flex items-center justify-center z-50 pointer-events-auto p-2 sm:p-4">
            <div className="w-full max-w-md bg-[rgba(12,14,16,0.95)] border-2 border-[#4FD1FF]/40 rounded-[16px] p-6 shadow-[0_0_35px_rgba(79,209,255,0.15)]">
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-black text-[#E5E7EB]">{t("g.sign.title")}</h2>
                ...
                {mode === "choice" && (
                    <div className="flex gap-3">
                        ...
                        >
                            ...
                            <span className="text-sm font-bold text-[#E5E7EB]">{t("g.sign.typeText")}</span>
                        </button>
                        <button
                        ...
                        >
                            ...
                            <span className="text-sm font-bold text-[#E5E7EB]">{t("g.sign.draw")}</span>
                        </button>
                    </div>
                )}
                ...
                {mode === "text" && (
                    <>
                        <textarea
                            ...
                            placeholder={t("g.sign.placeholder")}
                            rows={4}
                            autoFocus
