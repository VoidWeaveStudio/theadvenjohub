            ...
            <div className="w-full max-w-md bg-[rgba(12,14,16,0.95)] border-2 border-[#D4AF50]/40 rounded-[16px] p-6 shadow-[0_0_35px_rgba(212,175,80,0.15)]">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-lg font-black text-[#E5E7EB]">{t("g.arena.title")}</h2>
                    </div>
                ...
                {summary ? (
                    <div className="space-y-4">
                        <div className="text-center py-4">
                            <div className="text-[#8B8F98] text-xs tracking-widest uppercase">
                                {summary.reason === "candle_lost" ? t("g.arena.candleOut") : t("g.arena.partyFell")}
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div className="bg-[rgba(255,209,102,0.08)] border border-[#FFD166]/25 rounded-lg px-3 py-2 text-center">
                                ...
                                <div className="text-[#8B8F98] text-[10px] tracking-wider">{t("g.stat.ashCaps")}</div>
                            </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <ul className="text-[#8B8F98] text-xs space-y-1.5">
                            <li>• {t("g.arena.ruleBoss")}</li>
                            <li>• {t("g.arena.ruleRevive")}</li>
                            <li>• {t("g.arena.ruleNoLoot")}</li>
                            <li>• {t("g.arena.ruleRewards")}</li>
                        </ul>
                        ...
                        <div className="flex items-center gap-2 text-[#8B8F98] text-xs">
                            ...
                            <span>
                                {party.partyId
                                    ? t("g.arena.partyTogether", { count: partySize })
                                    : t("g.arena.alone")}
                            </span>
                        </div>
                        ...
                        {bestWave > 0 && (
                            <div className="flex items-center gap-2 text-[#8B8F98] text-xs">
                                <Trophy className="w-3.5 h-3.5 text-[#FFD166]" />
                                <span>{t("g.arena.best", { wave: bestWave })}</span>
                            </div>
                        )}
                        ...
                        {inRun ? (
                            <div className="space-y-2">
                                <div className="text-center text-[#4ADE80] text-sm font-bold">
                                    {arena.phase === "prep" ? t("g.arena.aboutToBegin") : t("g.arena.waveInProgress", { wave: arena.wave })}
                                </div>
                                <button
                                    onClick={onLeave}
                            ...
                            >
                                {t("g.arena.catchBreath", { time: formatCooldown(cooldownLeft) })}
                            </button>
                        ) : (
                            <div className="space-y-2">
                                <button
